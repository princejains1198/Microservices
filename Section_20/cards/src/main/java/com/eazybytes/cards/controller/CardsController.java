...
public class CardsController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(
                    responseCode = "404",
                    description = "Not Found - Cards details not found for the given mobile number"
            ),
            @ApiResponse(
                    responseCode = "500",
                    description = "Internal Server Error"
...
//        log.debug("eazyBank-correlation-id found : {}", correlationId);
        log.debug("Fetching card details method start");
        CardsDto cardsDto = cardsService.fetchCardDetails(mobileNumber);
        if (cardsDto == null) {
            log.debug("Card details method end");
            return ResponseEntity.status(HttpStatus.NO_CONTENT).build();
        } else {
            log.debug("Fetching card details method end");
