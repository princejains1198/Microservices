...
public class LoansController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(
                   responseCode = "404",
                    description = "Not Found - Loan details not found for the given mobile number"
            ),
            @ApiResponse(
                    responseCode = "500",
                    description = "Internal Server Error"
...
//        log.debug("eazyBank-correlation-id found : {}", correlationId);
        log.debug("Fetching loan details method start");
        LoansDto loansDto = loansService.fetchLoansDetails(mobileNumber);
        if(loansDto == null){
            log.debug("Fetching loan details method end");
